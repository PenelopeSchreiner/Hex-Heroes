...
func generate_map():
	...
	while land_percent < 0.4 and tries > 0:
		...
		generateBaseNoise(heat_params, rain_params, elv_params)
...
func generateBaseNoise(heatParams : Array, rainParams : Array, elvParams : Array):
...
func get_percent_land() -> float:
	...
	for cell in used_cells:
		if map.get_cell_atlas_coords(0, cell) != Vector2i(1, 0):
			land_cells += 1
